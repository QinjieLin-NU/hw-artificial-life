...
class NEURON: 
    ...
    def Update_Hidden_Or_Motor_Neuron(self, neurons, synapses):
        self.Set_Value(0) 
        name = self.Get_Name()
        for s_k in synapses.keys():
            (pre_synapse, post_synapse) = s_k
            if post_synapse == name:
                synapse_weight, presyn_value = synapses[s_k].Get_Weight(), neurons[pre_synapse].Get_Value()
                update_value = self.Allow_Presynaptic_Neuron_To_Influence_Me(synapse_weight, presyn_value)
                self.Add_To_Value(update_value)
        self.Threshold()

    def Allow_Presynaptic_Neuron_To_Influence_Me(self, synapse_weight, presyn_value):
        return synapse_weight*presyn_value
