...
class SOLUTION:
  def __init__(self, inputId, urdfId, seedId, envName):
    self.urdfId = urdfId
    self.seedId = seedId
    self.envName = envName
    self.urdf_file = f"./data/{self.envName}/seed{self.seedId}/body{self.urdfId}/body.urdf"
  ...
  def Start_Simulation(self,directOrGUI='DIRECT'):
    self.Generate_Brain()
    os.system(f"python3 simulate.py {directOrGUI} {self.myID} {self.urdfId} {self.seedId} {self.envName} > /dev/null 2>&1 &")
    # os.system(f"python3 simulate.py {directOrGUI} {self.myID} {self.urdfId} {self.seedId} {self.envName} &")

  def Wait_For_Simulation_To_End(self):
    while not os.path.exists(f"./data/{self.envName}/seed{self.seedId}/body{self.urdfId}/fitness{self.myID}.txt"):
      time.sleep(0.001)
    time.sleep(0.001)
    with open(f"./data/{self.envName}/seed{self.seedId}/body{self.urdfId}/fitness{self.myID}.txt", "r") as f:
      self.fitness = float(f.read())
    os.system(f"rm ./data/{self.envName}/seed{self.seedId}/body{self.urdfId}/fitness{self.myID}.txt")
  ...
  def Generate_Brain(self,prefix=None):
    if prefix is None:
      ...
      file_prefix = prefix
    #generate brain
    pyrosim.Start_NeuralNetwork(f"./data/{self.envName}/seed{self.seedId}/body{self.urdfId}/brain{file_prefix}.nndf")
    neuron_id = 0
