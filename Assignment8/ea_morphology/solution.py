...
class SOLUTION:
  def __init__(self, m_id, seed_id):
    self.m_id = m_id
    self.seed = seed_id
    self.urdf_folder = f"./data/seed{self.seed}/body{m_id}"
  ...
  def Set_ID(self,input_id):
    self.m_id = input_id
    self.urdf_folder = f"./data/seed{self.seed}/body{self.m_id}"
  ...
  def Start_Simulation(self,directOrGUI):
    self.Generate_URDF()
    os.system(f"python3 search_brain.py {self.m_id} {self.seed} > /dev/null 2>&1 &")
    # os.system(f"python3 search_brain.py {self.m_id} {self.seed} &")
    return
  
  def Replay_Best(self):
    os.system(f"python3 replay_brain.py best {self.m_id} {self.seed} > /dev/null 2>&1 &")
    return
