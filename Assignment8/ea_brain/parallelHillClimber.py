...
class PARALLEL_HILL_CLIMBER:
  def __init__(self, urdfId, seedId):
    os.system(f"rm ./data/seed{seedId}/body{urdfId}/brain*.nndf")
    os.system(f"rm ./data/seed{seedId}/body{urdfId}/fitness*.txt")
    os.system(f"rm ./data/seed{seedId}/body{urdfId}/tmp*.txt") 
    self.parents = {}
    ...
    for i in range(c.populationSize):
      self.parents[i] = SOLUTION(self.nextAvailableID, self.urdfId, self.seedId)
  ...
  def Save_Best(self):
    ...
    print(f"best parent of seed-{self.seedId}:",sorted_parents[-1][1].fitness)
    # plt.plot(self.average_fitness_history), plt.ylabel("Average fitness"), plt.xlabel("generation"), plt.show()
    with open(f"./data/seed{self.seedId}/body{self.urdfId}/best_fitness.txt", "w") as f:
